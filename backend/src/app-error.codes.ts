export const ErrorCode = {
  GEMINI_API_KEY_NOT_DEFINED: 'GEMINI_API_KEY_NOT_DEFINED',
  GEMINI_EMPTY_RESPONSE: 'GEMINI_EMPTY_RESPONSE',
  INVALID_CREDENTIALS: 'INVALID_CREDENTIALS',
  INVALID_OR_EXPIRED_REFRESH_TOKEN: 'INVALID_OR_EXPIRED_REFRESH_TOKEN',
  PROJECT_ACCESS_DENIED: 'PROJECT_ACCESS_DENIED',
  PROJECT_ALREADY_ACTIVE: 'PROJECT_ALREADY_ACTIVE',
  PROJECT_ALREADY_INACTIVE: 'PROJECT_ALREADY_INACTIVE',
  PROJECT_MUST_BE_INACTIVE_TO_DELETE: 'PROJECT_MUST_BE_INACTIVE_TO_DELETE',
  PROJECT_NOT_FOUND: 'PROJECT_NOT_FOUND',
  REFRESH_TOKEN_MISSING: 'REFRESH_TOKEN_MISSING',
  TICKET_NOT_FOUND: 'TICKET_NOT_FOUND',
  USER_EMAIL_ALREADY_EXISTS: 'USER_EMAIL_ALREADY_EXISTS',
  USER_NOT_FOUND: 'USER_NOT_FOUND',
  USER_ALREADY_INACTIVE: 'USER_ALREADY_INACTIVE',
  USER_ALREADY_ACTIVE: 'USER_ALREADY_ACTIVE',
  USER_INACTIVE: 'USER_INACTIVE',
  USER_NOT_PROJECT_MEMBER: 'USER_NOT_PROJECT_MEMBER',
  USER_MUST_BE_INACTIVE_TO_DELETE: 'USER_MUST_BE_INACTIVE_TO_DELETE',
  UNAUTHORIZED: 'UNAUTHORIZED',
} as const;
